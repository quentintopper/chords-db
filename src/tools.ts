import chords from "./db/guitar/chords";
...
const processChord = (suffixes, tuning) =>
  suffixes.map((suffix) => {
    return {
      ...suffix, 
      ...processPositions(suffix.positions, tuning),
    };
  });

function foldr<A, B>(list: A[], initial: B, fn: (item: A, acc: B) => B): B {
  let acc = initial;
  list.forEach((item) => {
    acc = fn(item, acc);
  });
...
const processChords = (chords, tuning) => {
  return {
    ...foldr(Object.keys(chords), {}, (chord, acc) => {
      return {
        ...acc,
        [chord]: processChord(chords[chord], tuning),
      };
    }),
  };
}
